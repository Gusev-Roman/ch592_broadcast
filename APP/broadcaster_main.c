...
void GPIOA_IRQHandler(void)
{
    ...
    if(is_sleep == 2){
        ...
        */
        ...
        /*
        первый сигнал может прийти в неспящем режиме - если чип разбужен соседним счетчиком.
        тут важно корректное значение предыдущего времени - если ноль, произойдет счет
        но резет не произойдет (нужно сбросить _wks чтобы продлить активность, чтобы процессор не заснул на дребезге)
        */
        if(f & GPIO_Pin_4){ // update cold
