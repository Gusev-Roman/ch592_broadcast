 ...
 * @fn      HSECFG_Current
 *
 * @brief   HSECrystal Bias Current Configuration
 *
 ...
 *
 * @param   cali_Lv - Calibration level selection    Level_32 ：2.4ms   1000ppm (32M Frequency)  1100ppm (60M Frequency)
 *                                   Level_64 ：4.4ms   800ppm  (32M Frequency)  1000ppm (60M Frequency)
 *                                   Level_128 ：8.4ms  600ppm  (32M Frequency)  800ppm  (60M Frequency)
 *
 * @return  none
 */
void Calibration_LSI(Cali_LevelTypeDef cali_Lv)
{
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            SYS_RecoverIrq(irqv);
            while(!(R8_OSC_CAL_CTRL & RB_OSC_CNT_HALT));
            i = R16_OSC_CAL_CNT; // Real-time calibration sampling value
        ...
        // Fine Tuning
        // After configuring the fine-tuning parameters, discard the 2 captured values ​​(software behavior) and judge that there is already one, so only keep one here
        sys_safe_access_enable();
        R8_OSC_CAL_CTRL &= ~RB_OSC_CNT_TOTAL;
 ...
 * @fn      RTCInitTime
 *
 * @brief   RTC clock initializes current time
 *
 ...
 * @param   s       - Configuration seconds，MAX_S = 59
 *
 * @return  none
 */
 ...
 *
 * @param   cyc - The trigger interval relative to the current time, based on the number of LSE/LSI clock cycles
 *
 * @return  none
 */
